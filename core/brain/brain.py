...
class JarvisBrain:
    ...
    def __init__(self, config_path: str = "config/default_config.yaml"):
        ...
        self.voice_manager = VoiceManager(self.config.get("voice", {}))
        self.reasoning_planner = ReasoningPlanner(self.config.get("reasoning", {}))
    ...
    async def process_input(self, user_input: str) -> str:
        ...
        # 2. Check if this is a reasoning task
        if self.reasoning_planner.is_reasoning_task(user_input):
            response = await self.reasoning_planner.analyze(user_input)
        else:
            # 3. Use RAG to retrieve relevant information
            context = await self.rag_manager.retrieve_relevant_context(user_input)
            
            # 4. Use agent to process the input and generate response
            response = await self.agent_manager.process_input(user_input, context)
        
        # 5. Update memory with the response
        await self.memory_manager.add_interaction(response)
        
        return response
    
    async def process_voice_input(self, duration: float = 5.0) -> str:
        """
        Process voice input and return a response.
        Records audio, converts to text, processes it, and speaks the response.
        """
        try:
            # 0. Give introduction
            introduction = "Hi, I am Jarvis. What can I do for you today?"
            await self.voice_manager.text_to_speech(introduction)
            
            # 1. Record audio
            audio_file = await self.voice_manager.record_audio(duration)
            
            # 2. Convert speech to text
            text = await self.voice_manager.speech_to_text(audio_file)
            if not text:
            ...
            await self.voice_manager.text_to_speech(response)
